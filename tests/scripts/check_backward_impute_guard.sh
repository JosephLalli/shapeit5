#!/usr/bin/env bash
# Simple regression check: run the supersite backward parity test and look for the IMPUTE guard skip message
set -euo pipefail
...
LD_LIBRARY_PATH=$HOME/.linuxbrew/lib:/usr/local/lib "$REPRO_TEST" 2>&1 | tee /tmp/check_backward_impute_guard.out
