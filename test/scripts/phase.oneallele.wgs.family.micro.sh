...
scaffold_bcf="$tmp_dir/target.scaffold.oneallele.micro.wgs.family.bcf"
scaffold_stats="$tmp_dir/target.scaffold.oneallele.micro.wgs.family.stats"
scaffold_debug="$tmp_dir/target.scaffold.oneallele.micro.wgs.family.debug"
output_bcf="$tmp_dir/target.phased.oneallele.micro.wgs.family.bcf"

# Phase common variants with MICRO mode
...
log_file="$tmp_dir/phase_rare.family.micro.log"
if ../phase_rare/bin/phase_rare \
  ...
  echo "Rare variant phasing (MICRO, family) completed successfully"
else
...
filtered_bcf="$tmp_dir/target.phased.oneallele.micro.wgs.family.filtered.bcf"
...
assert_same_md5 "$scaffold_bcf" "phase.wgs.family.micro.common"
assert_same_md5 "$filtered_bcf" "phase.wgs.family.micro"
